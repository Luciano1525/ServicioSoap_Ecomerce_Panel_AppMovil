    ...
    public function ver()
{
    ...
    error_log("ID de venta: $id");

    $data = $this->model->ListaVentas($id);
    $cliente = $this->model->PdfCliente($id);

    if (!$config) {
        die('Error: No se pudieron obtener los datos de configuración.');
    }
    if (!$data) {
        die('Error: No se pudieron obtener los datos de la venta.');
    }
    if (!$cliente) {
        die('Error: No se pudieron obtener los datos del cliente.');
    }

    error_log("Datos de configuración: " . print_r($config, true));
    error_log("Datos de la venta: " . print_r($data, true));
    error_log("Datos del cliente: " . print_r($cliente, true));

    $this->views->getView($this, "VerVentas", [
        'data' => $data,
        'config' => $config,
        'cliente' => $cliente
    ]);
